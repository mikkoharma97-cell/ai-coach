...
import { resolveCoachDayModel } from "@/lib/coachEngine";
import {
...
export function TodayView() {
  ...
  /** V1A: ajetaan rinnalla; UI käyttää edelleen alla olevaa todayDisplayBase -polkua. */
  const coachDayModel = useMemo(() => {
  ...
  const feedbackLine = useMemo(() => {
    ...
    if (isCompleted && !justFinishedWorkoutSession) {
      return t("todayView.dayCompletedHint");
    }
    return null;
  }, [
    ...
    <>
      <main className="coach-today-depth flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden pt-1.5 sm:pt-2">
        <Container
          ...
          data-coach-mode={coachDayModel?.mode}
        >
