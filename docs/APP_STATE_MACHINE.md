...
- [Navigointiaudit](NAVIGATION_AUDIT.md) — kaikki `router` / `window.location` -kohdat.
- [ADR 001: asiakasnavigointi](adr/001-client-navigation.md) — `replace` vs `push` vs täysi lataus.
- Coach-reittien kevyt sisältöanimaatio: [`src/app/(coach)/template.tsx`](../src/app/(coach)/template.tsx) (ei navia).
