...
**Näkyvä badge:** `HÄRMÄ22` (`VersionBadge` → `src/config/version.ts` → `COACH_RELEASE_LABEL`). Asetuksissa: `BuildMarkerLine` näyttää `HÄRMÄ22 · BUILD v22` + semver-aika.
...
**HÄRMÄ16 distribution:** `docs/harma16_distribution.md` — `appInfo`, meta, PWA, screenshot placeholder.

**HÄRMÄ17 QA:** `docs/harma17_qa.md` — `npm run qa:routes`, build-smoke, tunnetut rajat.

**HÄRMÄ18 sales demo:** `docs/harma18_sales_demo.md` — `/demo` myyntipuhe + polku + Progress-osio.

**HÄRMÄ19 AI engine:** `docs/harma19_ai_engine.md` — `buildCoachAiEngineResult`, Today käyttää yhtä päätöspolkua.

**HÄRMÄ20 notifications:** `docs/harma20_notifications.md` — opt-in selainmuistutukset, treeni / ruoka / päivä kesken.

**HÄRMÄ21 simple settings:** `docs/harma21_simple_settings.md` — `/preferences` ensin kevyt, loput *Lisää asetuksia*.

Tilat: **verified** = toteutettu ja linjassa auditin kanssa · **partial** = osin · **planned** = suunniteltu · **missing** = ei toteutettu.

### Evidence Sync Engine (HÄRMÄ33)
...
| **Versio** | `src/config/version.ts` — `APP_VERSION` v22, `HARMÄ_BUILD` 22 |
